...
import { ListGarantiasComponent } from '../../recaudacion/recibo-cobro/list-garantias/list-garantias.component';
import { ActivosFinancierosService } from './activos-financieros.service';
...
export class ActivosFinancierosComponent implements OnInit {
  ...
  constructor(
    ...
    private apiSrv: ActivosFinancierosService,
  ...
  fillCatalog() {
    ...
    this.apiSrv.getCatalogs(data).subscribe(
      (res) => {
        console.log(res);
        this.catalog = res["data"]["ACTIVO_FINANCIEROS"];
  ...
  createRecDocumento() {
    if (this.permissions.guardar == "0") {
      this.toastr.warning("No tiene permisos emitir Activos Financieros.", this.fTitle);
    } else {
      ...
      if(
        ...
        this.toastr.info("Debe ingresar el valor del Activo Financiero");
        return;
      }
      // if(this.difCobroPago!=0){
      //   this.toastr.info("Debe pagar el valor completo de la garantía");
      //   return;
      // }
      ...
      Swal.fire({
        icon: "warning",
        title: "¡Atención!",
        text:  'Está a punto de emitir un activo financiero. ¿Desea continuar?',
        ...
        if (result.isConfirmed) {
          this.msgSpinner = 'Generando activos financieros...';
          this.lcargando.ctlSpinner(true);
         ...
         // this.documento.fk_contribuyente = this.contribuyenteActive.id_cliente;
          ...
          this.apiSrv.setGarantia(data).subscribe(
            ...
            (error) => {
              ...
              Swal.fire({
                icon: "error",
                title: "Error al generar la garantía",
  ...
  actualizarActivos(){
    Swal.fire({
      icon: "warning",
      title: "¡Atención!",
      text: 'Está a punto de actualizar un activo financiero. ¿Desea continuar?',
      showCloseButton: true,
      showCancelButton: true,
      showConfirmButton: true,
      cancelButtonText: "Cancelar",
      ...
      if (result.isConfirmed) {
        this.msgSpinner = 'Actualizando activos financieros...';
        this.lcargando.ctlSpinner(true);
        
        let data = {
          id: this.documento.id_documento,
        ...
        this.apiSrv.setUpdateActivosFijos(data).subscribe(
          (res)=>{
            console.log(res);
            this.lcargando.ctlSpinner(false);
            this.movimientoRemove = true;
  ...
  imprimirReporte() {
    console.log(this.documento.id_documento)
      ...
      window.open(environment.ReportingUrl + "rpt_comprobante_activo_financiero.pdf?&j_username=" + environment.UserReporting + "&j_password=" + environment.PasswordReporting + "&id_documento=" + this.documento.id_documento, '_blank')
