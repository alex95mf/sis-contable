...
export class NotaCreditoComponent implements OnInit {
  ...
  constructor(
    ...
    private apiSrv: NotaCreditoService,
    ...
    private cierremesService: CierreMesService
      ...
      this.commonVrs.selectContribuyenteCustom.asObservable().subscribe(
        (res) => {
          ...
          else if(res.valid == 'Endoso'){
            this.contribuyenteActiveEndosa = res;
          
          this.notasDisabled = false;
          // this.getLiquidaciones();
          this.vmButtons[0].habilitar=false;
          }
      ...
      this.commonVrs.selectRecDocumento.asObservable().subscribe(
        (res) => {
          ...
          this.documento = res;
          this.documento.tipo_nota = res.tipo;
          ...
          if(this.documento.tipo_nota=='E'){
            this.contribuyenteActiveEndosa = res.contribuyente_2;
          }
  ...
  selectDocPago() {
    if(this.formaPago.valor=='NOTA CREDITO'){
      this.expandNotasCredito();
    } 
  }
  ...
  expandNotasCredito() {
    ...
    modal.componentInstance.contr = this.contribuyenteActiveEndosa;
  ...
  pagoDirecto(doc: any) {
    ...
    let nuevo = {
      id_documento_forma_pago: doc.id_documento,
...
validaDataGlobal() {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (this.permissions.guardar == "0") {
      this.toastr.warning("No tiene permisos emitir Notas de Credito.", this.fTitle);
      flag = true;
    }else if(this.totalCobro <=0 || this.totalCobro == undefined){
      this.toastr.warning("Debe ingresar un Monto.", this.fTitle);
      flag = true;
    }else if(this.documento.motivo=="" || this.documento.motivo== undefined){
      this.toastr.warning("Debe ingresar un Motivo.");
      flag = true;
    }
    else if(this.documento.tipo_nota == 0 || this.documento.tipo_nota == undefined){
      this.toastr.warning("Debe seleccionar un Tipo.");
      flag = true;
    }else if(this.documento.tipo_nota == 'E' ){
      if(this.contribuyenteActiveEndosa.razon_social=="" || this.contribuyenteActiveEndosa.razon_social==undefined){
        this.toastr.warning("Debe escoger el Contribuyente que endosara la nota.");
        flag = true;
      }else if(this.formaPago == 0 || this.formaPago == undefined){
        this.toastr.warning("Debe seleccionar la forma de pago.");
        flag = true;
      }else if(this.contribuyenteActiveEndosa.razon_social=="" || this.contribuyenteActiveEndosa.razon_social==undefined){
        this.toastr.warning("Debe escoger el Contribuyente que endosara la nota.");
        flag = true;
      }else if(this.formaPago == 0 || this.formaPago == undefined){
        this.toastr.warning("Debe seleccionar la forma de pago.");
        ...
        this.pagos.forEach(p => {
          console.log(parseFloat(p.valor) + '-'+(p.nuevo_saldo))
          if(parseFloat(p.valor) > parseFloat(p.nuevo_saldo)){
            this.toastr.warning("El valor no puede ser mayor al saldo de la nota seleccionada.");
            flag = true;
          }else if(parseFloat(p.valor) < this.totalCobro){
            this.toastr.warning("El valor debe cubrir el monto total de la nota de crédito");
            flag = true;
          }else if(parseFloat(p.valor) > this.totalCobro){
            this.toastr.warning("Solo debe pagar el valor total de la nota de crédito");
            flag = true;
          }
        }
  ...
  createRecDocumento() {
    ...
    //   // else if(this.pagos.length>0 ){
    //   //   this.pagos.forEach(p => {
    //   //     if(p.valor > p.nuevo_saldo){
    //   //       this.toastr.warning("El valor a no puede ser mayor al saldo de la nota seleccionada.");
    //   //     }else if(p.valor < this.totalCobro){
    //   //       this.toastr.warning("El valor debe cubrir el monto total de la nota de crédito");
    //   //     }else if(p.valor > this.totalCobro){
    //   //       this.toastr.warning("Solo debe pagar solo el valor total de la nota de rédito");
    //   //     }
    //   //   }
   ...
   // else {
      ...
      Swal.fire({
        icon: "warning",
        title: "¡Atención!",
        text:'¿Está a punto de emitir una Nota de Crédito desea continuar?' ,
        ...
        if (result.isConfirmed) {
          ...
          let data = {
            ...
            if (res["data"][0].estado !== 'C') {
              (this as any).mensajeSpinner = 'Generando Nota de Credito...';
              this.lcargando.ctlSpinner(true);
              ...
              if(this.documento.tipo_nota=='E'){
                this.documento.fk_contribuyente_endoso = this.contribuyenteActiveEndosa.id_cliente;
              }else{
                this.documento.fk_contribuyente_endoso = null;
              }
              ...
              this.apiSrv.setNotaCredito(data2).subscribe(
                (res) => {
                  console.log(res);
                  
                  this.documento = res['data'];
                  this.documento.tipo_nota = res['data']['tipo'];
                  this.formReadOnly = true;
                  this.vmButtons[0].habilitar = true;
                  this.vmButtons[2].habilitar = false;
                  this.vmButtons[3].habilitar = false;
                ...
                (error) => {
                  ...
                  Swal.fire({
                    icon: "error",
                    title: "Error al generar la Nota de Crédito",
  ...
  restoreForm() {
    ...
    this.contribuyenteActiveEndosa = {
      razon_social: ""
    }
  ...
  console.log(this.documento.id_documento)
    if(this.documento.tipo_nota=='N'){
      window.open(environment.ReportingUrl + "rep_tesoreria_nota_credito.pdf?&j_username=" + environment.UserReporting + "&j_password=" + environment.PasswordReporting + "&id_documento=" + this.documento.id_documento , '_blank')
    }
    if(this.documento.tipo_nota=='E'){
      window.open(environment.ReportingUrl + "rep_tesoreria_endoso_nota_credito.pdf?&j_username=" + environment.UserReporting + "&j_password=" + environment.PasswordReporting + "&id_documento=" + this.documento.id_documento , '_blank')
    }
  }
  ...
  expandListContribuyentesNota() {
    ...
    // } else {
      ...
      modalInvoice.componentInstance.permissions = this.permissions;
      modalInvoice.componentInstance.verifyRestore = this.verifyRestore;
      modalInvoice.componentInstance.validacion = 'Nota';
    // }
  }
  expandListContribuyentesEndosa() {
    ...
    // } else {
      ...
      modalInvoice.componentInstance.permissions = this.permissions;
      modalInvoice.componentInstance.verifyRestore = this.verifyRestore;
      modalInvoice.componentInstance.validacion = 'Endoso';
    // }
  }
