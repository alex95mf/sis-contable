...
export class ListCruceConvenioComponent implements OnInit {
  ...
  cargarDocumentos(){
    this.mensajeSppiner = "Cargando listado de cruce convenios...";
    ...
    this.apiSrv.getRecDocumentosCruce(data).subscribe(
      (res) => {
  ...
  selectOption(data) {
    if (this.verifyRestore) {
      Swal.fire({
        icon: "warning",
        title: "¡Atención!",
        text: "¿Seguro que desea visualizar este Cruce Convenio? Los campos llenados y cálculos realizados serán reiniciados.",
