...
export class ModalLiquidacionesComponent implements OnInit {
  ...
  masterSelectedCO: boolean = false;
  masterIndeterminateCO: boolean = false;
  ...
  changePaginateCO({pageIndex}) {
    this.masterSelectedCO = false
    Object.assign(this.paginateCO, {page: pageIndex + 1});
    ...
    this.apiSrv.getLiqByContribuyente(data).subscribe(
      (res) => {
        ...
        array.forEach(e => {
          ...
          this.deudas.forEach(c => Object.assign(e, { aplica: e.id_liquidacion == c.id_liquidacion}))
          // this.checkIndetereminate(e)
        })
        this.resdata = array;
        //
        if(firstload){
    ...
    this.apiSrv.getLiqByContribuyente(dataCO).subscribe(
      (res) => {
        ...
        if(firstload){
          ...
          this.checkIndetereminateCoPage()
          this.lcargando.ctlSpinner(false);
        }
      },
      (error) => {
  ...
  aplica(dt){
    ...
    if(aplica){
      ...
      Object.assign(dt , {
        ...
        exoneraciones:dt.exoneraciones,
        servtecadmin:dt.servtecadmin,
        recargo:dt.recargo,
        coactiva:dt.coactiva
      })
  ...
  closeModal(data?:any) {
    ...
    let cobros = {
      titulos: this.resdata,
      cuotas: this.liquidacionesDt,
      cuotasAT: this.conveniosATDt,
    }
    this.commonVrs.cobros.next(cobros);
  ...
  selectAll() {
    ...
    this.resdata.map((item: any) => {
      Object.assign(item, { aplica: this.masterSelected})
      this.aplica(item)
    })
  }

  checkIndetereminate(dt) {
    const someSelected = this.resdata.reduce((acc, curr) => acc | curr.aplica, 0)
    const allSelected = this.resdata.reduce((acc, curr) => acc & curr.aplica, 1)
  ...
  selectAllCO() {
    this.masterIndeterminateCO = false
    this.liquidacionesDt.forEach((element: any) => {
      Object.assign(element, {aplica: this.masterSelectedCO})
      this.aplica(element)
    })
  }

  checkIndetereminateCoPage() {
    const someSelected = this.liquidacionesDt.reduce((acc, curr) => acc | curr.aplica, 0)
    const allSelected = this.liquidacionesDt.reduce((acc, curr) => acc & curr.aplica, 1)

    this.masterIndeterminateCO = !!(someSelected && !allSelected)
    this.masterSelectedCO = !!allSelected
