...
export class GarantiaComponent implements OnInit, OnDestroy {
  ...
  /* recargarCaja() {
    ...
    if(!this.cajaActiva || this.cajaActiva.fecha != moment().format('YYYY-MM-DD')){
      Swal.fire('Advertencia', 'Debe tener una caja activa para poder realizar cobros.', 'warning');
      this.formReadOnly = true
      return;
    }
  ...
  createRecDocumento() {
    if (this.permissions.guardar == "0") {
      this.toastr.warning("No tiene permisos emitir Garantías.", this.fTitle);
    } else {
      ...
      if(
        ...
        this.toastr.info("Debe ingresar el valor de la garantía");
        return;
      }
      ...
      Swal.fire({
        icon: "warning",
        title: "¡Atención!",
        text: this.superavit ? 'Está a punto de emitir una garantía con un superávit de '+this.documento.superavit+' ¿Desea continuar?' : "Está a punto de emitir una garantía ¿Desea continuar?",
        ...
        if (result.isConfirmed) {
          ...
          let data = {
            ...
            if (res["data"][0].estado !== 'C') {
              ...
              (this as any).mensajeSpinner = 'Generando Garantía...';
              this.lcargando.ctlSpinner(true);
