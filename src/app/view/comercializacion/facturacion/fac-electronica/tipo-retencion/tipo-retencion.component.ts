...
export class TipoRetencionComponent implements OnInit {
  ...
  procesoDual(valor:any){
    ...
    try {
        let datosEnviar = [];
        let emision:any = valor._retencion_cab._compras.num_documento.split("-");
        valor.lestab = emision[0].padStart(3, '0');
        valor.lptoEmi = emision[1].padStart(3, '0');
        valor.lsecuencial = emision[2].padStart(9, '0');

        let fechaEmision:any = valor._retencion_cab._compras.fecha.split("-");
        valor._retencion_cab._compras.fechaEmision = fechaEmision[2]+"/"+fechaEmision[1]+"/"+fechaEmision[0];
        valor._retencion_cab._compras.iva_porcentaje = this.validaciones.roundNumber(valor._retencion_cab._compras.iva_porcentaje, 2);

        valor._retencion_cab._compras.periodoFiscal = fechaEmision[1]+"/"+fechaEmision[0];
  ...
  enviarEmail(item:any){
    ...
    item.lNumeroDocumento = item._retencion_cab.numero;
    item.lNombreEstimado = item._retencion_cab._compras.proveedor.nombre_comercial_prov;
    ...
    item.lNombreCiaCedula = item._retencion_cab._compras.company.razon_social + " - " + item._retencion_cab._compras.company.ruc
    item.lObligadoContabilidad = item._retencion_cab._compras.company.obligado_contabilidad;
    item.lNumeroTelefono = item._retencion_cab._compras.company.celular;
    item.lDireccionEmpresa = item._retencion_cab._compras.company.direccion;
    item.lEmailEmpresa = item._retencion_cab._compras.company.email;
    item.lNombreVendedor = item._retencion_cab._compras.usuario.nombre;
    item.lEmailVendedor = item._retencion_cab._compras.usuario.email;
    item.lRazonSocialCliente = item._retencion_cab._compras.proveedor.razon_social;
    item.lIdentificacionCliente = item._retencion_cab._compras.proveedor.num_documento;
    item.lDireccionCliente = item._retencion_cab._compras.proveedor.direccion;
    item.lFechaEmisionInfo = item._retencion_cab._compras.fecha;
    ...
    let dataPresentar = {
      mensaje: "Ingresar correo electrónico para enviar el documento electrónico de tipo " + item._tipo_documento.nombre + ", con el codigo de acceso: " + item.codigo_acceso + ", de " + item._retencion_cab._compras.proveedor.nombre_comercial_prov + " - " + item._retencion_cab._compras.proveedor.num_documento,
