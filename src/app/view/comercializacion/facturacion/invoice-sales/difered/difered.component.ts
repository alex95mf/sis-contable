...
export class DiferedComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.vmButtons = [
      { orig: "btnInvoiceDiferedFac", paramAccion: "", boton: { icon: "fa fa-floppy-o", texto: "GUARDAR" }, permiso: true, showtxt: true, showimg: true, showbadge: false, clase: "btn btn-success boton btn-sm", habilitar: false },
      { orig: "btnInvoiceDiferedFac", paramAccion: "", boton: { icon: "fas fa-chevron-left", texto: "REGRESAR" }, permiso: true, showtxt: true, showimg: true, showbadge: false, clase: "btn btn-danger boton btn-sm", habilitar: false }
    ];
    /* console.log(this.diferedEdit,this.ammount);
    if (this.diferedEdit != null) {
 ...
 foo(num1, num2){ 
  ...
  async confirmSave(data) {
    Swal.fire({
      ...
      if (result.value) {
        this.commonVarServices.listenDiferedInvoice.next(data);
        this.closeModal();
      }
    })
  }
