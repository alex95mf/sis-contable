...
export class ModalRegContribuyenteComponent implements OnInit {
  ...
  constructor(
    ...
    private prestamoSrv: PrestamoService,
  ...
  buscarContribuyente() {
    ...
    if (this.tipoPersona != undefined) {
      this.prestamoSrv.getContribuyentesByFilter(data).subscribe(
        (res) => {
  ...
  fillCatalog() {
    ...
    this.prestamoSrv.getCatalogos(data).subscribe(
      (res) => {
        // console.log(res);
    ...
    this.prestamoSrv.getAgentRetencion({}).subscribe(res => {
  ...
  searchProvinces(event) {
    console.log(event);
    this.prestamoSrv.filterProvinceCity({ grupo: event }).subscribe(res => {
  ...
  saveContribuyente() {
    ...
    (this as any).mensajeSpinner = "Guardando Contribuyente";
    this.lcargando.ctlSpinner(true);
