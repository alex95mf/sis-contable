...
export class ModalBusquedaConstatacionComponent implements OnInit {
  ...
  constructor(
    ...
    private mdlSrv: ConstatacionFisicaBCAService,
  ...
  cargarConstatacion() {
    this.mensajeSppiner = "Cargando lista de Constatación física...";
    ...
    this.mdlSrv.getIngresosPorPeriodo(data).subscribe(
      (res) => {
        // console.log(data);
        console.log(res);
  ...
  Departamentos() {
    ...
    this.mdlSrv.listarDepartamentos({}).subscribe((res: any) => {
      res.map((data) => {
        console.log(res);
        let datos = {
  ...
  catalogo() {
    ...
    this.mdlSrv.listarCatalogo({}).subscribe((res: any) => {
      res.map((data) => {
