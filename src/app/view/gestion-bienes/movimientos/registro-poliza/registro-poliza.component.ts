...
import { ListBusquedaPrestamoComponent } from '../mantenimiento/list-busqueda-prestamo/list-busqueda-prestamo.component';
import { RegistroPolizaService } from './registro-poliza.service';
...
export class RegistroPolizaComponent implements OnInit {
  ...
  )
  {
    ...
    this.commonVarServices.selectPoliza.asObservable().subscribe(
      (res) => {
       // this.catalogo()
        console.log(res)
        this.id = res['id_registro_poliza']
        ...
        this.documento.fecha_inicio = res['fecha_inicio']
        this.documento.fecha_fin = res['fecha_fin']
        this.documento.tipo_poliza = res['tipo_poliza']
        this.documento.numero_poliza_contrato = res['numero_poliza_contrato']
        this.documento.valor_asegurado = res['valor_asegurado']
  ...
  saveMantenimiento(){
    ...
    Swal.fire({
      icon: "warning",
      title: "¡Atención!",
      text: "Está a punto de guardar una poliza ¿Desea continuar?",
  ...
  uploadFile(id_registro_poliza) {
    console.log(id_registro_poliza)
    let data = {
      //module: this.permissions.id_modulo,
      module:21,
      component: myVarGlobals.fGestBienesPoliza,
      identifier: id_registro_poliza,
    ...
    if(this.fileList.length!=0){
      for (let i = 0; i < this.fileList.length; i++) {
        this.UploadService(this.fileList[i], id_registro_poliza,data );
      }
    }
    this.fileList = undefined
    this.lcargando.ctlSpinner(false)
  ...
  catalogo() {
    ...
    this.invService.listarCatalogo({}).subscribe((res: any) => {
      console.log(res);
      res.map((data) => {
  ...
  editar(){
    ...
    Swal.fire({
      icon: "warning",
      title: "¡Atención!",
      text: "¿Seguro que desea editar este registro de póliza?",
      showCloseButton: true,
      showCancelButton: true,
      showConfirmButton: true,
      cancelButtonText: "Cancelar",
      ...
      if (result.isConfirmed) {
        ...
        let data = {
          ...
          tipo_poliza:this.documento.tipo_poliza,
          fecha:this.documento.fecha,
          fecha_inicio:this.documento.fecha_inicio,
          fecha_fin:this.documento.fecha_fin,
          valor_asegurado:this.documento.valor_asegurado,
          numero_poliza_contrato:this.documento.numero_poliza_contrato,
          observaciones:this.documento.observaciones,
          detalles:this.producto
        }
        this.invService.updatePoliza(data).subscribe(
          (res)=>{
           if(res["status"]==1){
            this.lcargando.ctlSpinner(false);
            Swal.fire({
  ...
  aprobar(){
     if (this.documento.observaciones == "" || this.documento.observaciones == undefined) {
      this.toastr.info("Debe editar e ingresar una observación")
      return;
    }
     else if (this.documento.numero_poliza_contrato == "" || this.documento.numero_poliza_contrato == undefined) {
      this.toastr.info("Debe editar e ingresar un número de póliza")
      return;
    }
    else if (this.documento.tipo_poliza == "" || this.documento.tipo_poliza == undefined) {
      this.toastr.info("Debe editar e ingresar un tipo de póliza")
      return;
    }
    else if (this.documento.valor_asegurado <= 0 ) {
      this.toastr.info("Debe editar e ingresar un valor asegurado válido")
      return;
    }
