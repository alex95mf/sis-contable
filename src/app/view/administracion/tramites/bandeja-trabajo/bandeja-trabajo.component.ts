...
export class BandejaTrabajoComponent implements OnInit {
  ...
  cargarTicketsGlobal() {
    ...
    this.ticketSrv.getTicketsGlobal(this.dataUser['id_rol'],data).subscribe(
      (res) => {
        console.log(res);
  ...
  // Abre el modal y guarda el trámite
  openRetornarModal(dt: any) {
    this.tramiteSeleccionado = dt;
    this.motivoRetorno = ''; // Resetear el motivo al abrir el modal
    this.modalSrv.open(this.retornarModal);
    console.log(this.tramiteSeleccionado)
  }

  // Valida antes de confirmar
  validarRetorno(modal: any) {
    if (!this.motivoRetorno || this.motivoRetorno.trim() === '') {
      this.toastr.error('Debe ingresar el motivo del retorno.', 'Error', {
        timeOut: 3000,
  ...
  private confirmarRetorno() {
    let data = {
      id_seguimiento: this.tramiteSeleccionado.id_tramite_seguimiento,
      id_seguimiento_anterior: this.tramiteSeleccionado.fk_tramite_seguimiento_anterior,
    ...
    this.ticketSrv.retornarTramiteSeguimiento({data}).subscribe(
      ...
      (res) => {
        ...
        this.lcargando.ctlSpinner(false);
        this.cargarTicketsGlobal();
        this.toastr.success('Trámite retornado correctamente.', 'Éxito');
      },
      (error) => {
        this.lcargando.ctlSpinner(false);
        this.toastr.info(error.error.message);
  ...
  getEstadoLabel(estadoValue: string): string {
    const estado = this.estadoList.find(e => e.value === estadoValue);
    return estado ? estado.label : 'DESCONOCIDO';
  }
  
  getEstadoIconClass(estadoValue: string): string {
    const baseClass = 'fas fa-circle';
    switch(estadoValue) {
      case 'P': return `${baseClass} text-primary`;
      case 'C': return `${baseClass} text-danger`;
      case 'R': return `${baseClass} text-warning`; // o el color que prefieras para RETORNADO
      default: return `${baseClass} text-secondary`;
    }
  }
