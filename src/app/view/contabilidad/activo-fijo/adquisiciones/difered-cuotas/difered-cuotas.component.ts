...
export class DiferedCuotasComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.vmButtons = [
      { orig: "btnDifAdqAcFij", paramAccion: "", boton: { icon: "fa fa-floppy-o", texto: "GUARDAR" }, permiso: true, showtxt: true, showimg: true, showbadge: false, clase: "btn btn-success boton btn-sm", habilitar: false },
      { orig: "btnDifAdqAcFij", paramAccion: "", boton: { icon: "fas fa-chevron-left", texto: "REGRESAR" }, permiso: true, showtxt: true, showimg: true, showbadge: false, clase: "btn btn-danger boton btn-sm", habilitar: false }
    ];
 ...
 foo(num1, num2){ 
  ...
  async confirmSave(data) {
    Swal.fire({
      ...
      if (result.value) {
        this.commonVarServices.listenDiferedActivoFijo.next(data);
        this.closeModal();
      }
    })
  }
