...
import { ShowLiquidacionesComponent } from './show-liquidaciones/show-liquidaciones.component';
...
export class LiquidacionesComponent implements OnInit {
  ...
  constructor(
    ...
    this.commonVarSrv.listenLiquidacionImp.asObservable().subscribe(res => {
      ...
      if (!this.dsbGlb) {
        ...
        this.numSec = this.secGlobal[0]['codigo'] + "-" + res.num_doc.toString().padStart(10, '0');
        this.dataProducto = res.detalle;
        this.dataProductoAux = res.detalle_aux;
        ...
        this.importPed['total_general'] = parseFloat(res['total_general']);
        this.importPed.total_gasto = parseFloat(res.total_gasto);
        this.importPed.total_fob = parseFloat(res.total_fob);
        this.importPed.total_seguro = parseFloat(res.total_seguro);

        if (/* this.importPed.total_transp == 0 && this.importPed.total_seguro == 0 */this.importPed.transport_Aux_val <= 0 && this.importPed.seguro_Aux_val <= 0) {
          res['gastos'].forEach(element => {
          ...
          this.dataProCierre = [];
          this.dataProCierre = res.detalle_cierre;
  ...
  getPermisions() {
    ...
    let data = {
      codigo: myVarGlobals.fImpLiquidacion,
      id_rol: id_rol
    }
  ...
  async validateCierreLiqImp() {
    if (this.permisions.editar == "0") {
      ...
      if (this.rubrosFaltantes.length > 0) {
        Swal.fire({
          title: 'Error!!',
          text: "Faltan gastos que son obligatorios: " + this.rubrosFaltantes,
          icon: 'error',
          confirmButtonColor: '#d33',
          confirmButtonText: 'Ok'
        }).then((result) => {
