...
export class ShowNotasCreditoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.vmButtons = [
      { orig: "btnNcC", paramAccion: "", boton: { icon: "fas fa-chevron-left", texto: "REGRESAR" }, permiso: true, showtxt: true, showimg: true, showbadge: false, clase: "btn btn-danger boton btn-sm", habilitar: false }
    ];
  ...
  getTableAccounts() {
    ...
    this.accSrv.getNotasCredito({ id_document: this.id_document,type:"Cliente" })
      .subscribe(res => {
        this.validaDt = true;
  ...
  editarNC(dt) {
    this.commonVarSrvice.setNotasCreditoVenta.next(dt);
    this.closeModal();
  }
  ...
  deleteNC(dt) {
    if (dt.isModule == 1) {
      Swal.fire({
        title: 'Atención!!',
        text: "Esta nota de crédito no puede ser eliminada!!",
        icon: 'error',
      ...
      Swal.fire({
        ...
        preConfirm: (informacion) => {
          if (informacion == "") {
            ...
            this.accSrv.deleteNotaCredito(data).subscribe(res => {
  ...
  nexStatus(dt) {
    if (this.editar == "0") {
      ...
      let permission = this.commonService.filterPermissionStatus((dt.filter_doc == null || dt.filter_doc == undefined) ? 1 : dt.filter_doc, this.id_document);
      this.NexPermisions = this.commonService.filterNexStatus((dt.filter_doc == null || dt.filter_doc == undefined) ? 1 : dt.filter_doc, this.id_document);
      if (permission) {
        this.confirmSave("Seguro desea cambiar de estado de la nota de crédito?", "MOD_NCV", dt);
      } else {
  ...
  modNCV(ncv) {
    ...
    let data = {
      ...
      id: ncv.id,
      ip: this.commonService.getIpAddress(),
      accion: `Cambio de estado de la nota de crédito número ${ncv.secuencia_doc.toString().padStart(10, '0')} `,
      id_controlador: this.module_comp,
      filter_doc: this.NexPermisions,
      id_document: this.id_document,
      abbr_doc: this.dataUser.permisos_doc.filter(e => e.fk_documento == this.id_document)[0].codigo,
      notifycation: `Se ha cambiado de estado a la nota de crédito ${ncv.secuencia_doc.toString().padStart(10, '0')} por el usuario ${this.dataUser.nombre}`,
