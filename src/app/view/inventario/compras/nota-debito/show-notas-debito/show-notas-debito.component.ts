...
export class ShowNotasDebitoComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.vmButtons = [
      { orig: "btnNdP", paramAccion: "", boton: { icon: "fas fa-chevron-left", texto: "REGRESAR" }, permiso: true, showtxt: true, showimg: true, showbadge: false, clase: "btn btn-danger boton btn-sm", habilitar: false }
    ];
  ...
  getTableAccounts() {
    ...
    this.accSrv.getNotasDebito({ id_document: this.id_document,type:"Poveedor"})
      .subscribe(res => {
        this.validaDt = true;
