...
export class AnexosComponent implements OnInit {
  ...
  constructor(private proveedoresServicio: SuppliersService, private toastr: ToastrService, private router: Router, private zone: NgZone, private commonServices: CommonService,
    ...
    this.commonServices.anexosProvider.asObservable().subscribe(res => {
      let data = {
        anexos_delete: this.viewDelete,
      }
      this.commonServices.resAnexosProvider.next(data);
    })

    this.commonServices.saveProveedores.asObservable().subscribe(res => {
      if (this.filesSelect !== undefined) {
        ...
        res['component'] = myVarGlobals.fProveedores;
        res['identifier'] = res.identifier;
        res['id_controlador'] = myVarGlobals.fProveedores;
  ...
  showDataTableProveedores(params) {
    if (params.length > 0) {
