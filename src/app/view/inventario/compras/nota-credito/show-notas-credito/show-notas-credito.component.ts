...
export class ShowNotasCreditoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.vmButtons = [
      { orig: "btnNcP", paramAccion: "", boton: { icon: "fas fa-chevron-left", texto: "REGRESAR" }, permiso: true, showtxt: true, showimg: true, showbadge: false, clase: "btn btn-danger boton btn-sm", habilitar: false }
    ];
  ...
  getTableAccounts() {
    ...
    this.accSrv.getNotasCredito({id_document:this.id_document,type:"Proveedor"})
      .subscribe(res => {
        this.validaDt = true;
    ...
    if (dt.isModule == 1) {
      ...
      Swal.fire({
        ...
        preConfirm: (informacion) => {
          if(informacion == ""){
            this .toastr.error("Ingrese una descripción");
          }else{
            let data = {
              motivo: informacion,
              id_ncdv_cab: dt.id,
