...
@Component({
standalone: false,
  selector: 'app-generacion-idp-nomina',
  templateUrl: './generacion-idp-nomina.component.html',
  styleUrls: ['./generacion-idp-nomina.component.scss']
})
export class GeneracionIdpNominaComponent implements OnInit {

  @ViewChild(CcSpinerProcesarComponent, { static: false }) lcargando: CcSpinerProcesarComponent;
  @ViewChild("print") print!: ElementRef;
  fTitle = "Generación de IDP de Nómina";
  ...
  constructor(
    ...
    private genIdpSvr: GeneracionIdpNominaService,
    ...
    this.commonVrs.selectRecDocumento.asObservable().subscribe(
      (res) => {
        ...
        if(res.detalles.length > 0){
          res.detalles.forEach(e =>{
              let data = {
                ...
                fk_icp_origen_detalle: e.fk_icp_origen_detalle,
                partida: e.partida,
                denominacion: e.catalogo_presupuesto?.nombre ?? '',
                disponible: e.precio_cotizado,
                valor_solicitado: e.detalles_documento_icp?.precio_solicitado,
                saldo_actual:e.detalles_documento_icp?.saldo_actual,
                precio_aprobado:e.precio_solicitado,
                valor_idp: e.precio_solicitado,
                saldo_actual_idp:e.saldo_actual,
        ...
        this.vmButtons[4].habilitar = !(this.documento.estado == 'E') || this.documento.total < 0;
      }
    )
        ...
        if(res.detalles.length > 0){
          res.detalles.forEach(e =>{
              let data = {
                ...
                precio_aprobado: 0,
                valor_icp: e.detalles_documento_2?.precio_solicitado,
                saldo_icp: e.detalles_documento_2?.saldo_actual,
                saldo:e.saldo_actual,
              }
              this.solicitudDetalle.push(data)
          })
  ...
  createRecDocumento() {
    ...
    if (this.totalCobro <= 0 || this.totalCobro == undefined) message += '* Monto del IDP no puede ser menor o igual a 0.<br>';
    if (this.totalCobro > this.solicitud[0].saldo_actual) message += '* El monto no puede exceder al valor de ICP.<br>';
    ...
    Swal.fire({
      icon: "warning",
      title: "¡Atención!",
      text: '¿Está a punto de generar un IDP de Nómina desea continuar?',
      showCloseButton: true,
        ...
        let datos = {
          ...
          /* Validamos si el periodo se encuentra aperturado */
            if (res["data"][0].estado !== 'C') {
                  (this as any).mensajeSpinner = 'Generando IDP de Nomina...';
                  this.lcargando.ctlSpinner(true);
                  ...
                  this.documento.detalles = this.solicitudDetalle;
  ...
  totalPresupuesto(){
    this.totalCobro = this.solicitudDetalle.reduce((acc: number, curr) => {return acc + parseFloat(curr.valor_solicitado)}, 0)
  }
  ...
  modalSolicitud() {
    if (this.documento.periodo == undefined || this.documento.periodo == '') {
      ...
      const modal = this.modalService.open(ModalIcpNominaComponent, {
        size: "xl",
        backdrop: "static",
        windowClass: "viewer-content-general",
      })
