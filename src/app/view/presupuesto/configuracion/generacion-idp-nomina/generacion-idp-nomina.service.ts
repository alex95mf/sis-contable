...
export class GeneracionIdpNominaService {

  constructor(private api: ApiServices) { }
  
  setGeneracionIdpNomina(data) {
    return this.api.apiCall("presupuesto/set-generar-idp-nomina", "POST", data);
  }
  getGeneracionIdpNomina(data) {
    return this.api.apiCall("presupuesto/get-idp-nomina-generados", "POST", data);
  }
  ...
  getIcpNominaModal(data) {
    return this.api.apiCall('presupuesto/get-modal-icp-nomina', 'POST', data)
  }
  ...
  ajustarIdpNomina(id: number, data: any) {
    return new Promise((resolve, reject) => {
      this.api.apiCall(`presupuesto/ajustar-idp-nomina/${id}`, 'POST', data).subscribe(
        (res: any) => resolve(res.data),
        (err: any) => reject(err)
      )
    })
