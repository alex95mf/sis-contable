...
export class GeneracionIcpNominaComponent implements OnInit {
  ...
  constructor(
    ...
    private genIdpSvr: GeneracionIcpNominaService,
    private cierremesService: CierreMesService,
      ...
      this.commonVrs.selectRecDocumento.asObservable().subscribe(
        (res) => {
          ...
          if(res.detalles.length > 0){
            res.detalles.forEach(e =>{
                let data = {
                  ...
                  partida: e.partida,
                  denominacion: e.catalogo_presupuesto?.nombre ?? '',
                  disponible: e.precio_cotizado,
                  valor_solicitado: e.precio_solicitado,
                  saldo_actual: e.saldo_actual,
                  saldo: e.saldo_actual,
                  ajuste: e.ajuste
                }
                this.cedulaDetalle.push(data)
            })
          }
          ...
          if(res.programa != null){
            let dat = {
              id_catalogo: res.programa.id_catalogo,
              label:  res.programa.descripcion + '-' +  res.programa.valor
            }
            programa.push(dat)
          }
      ...
      this.genIdpSvr.presupuestoSelected$.subscribe(
        (res: any) => {


          this.agregaPresupuesto(res)
  ...
  totalSolicitud() {
    this.totalCobro = this.cedulaDetalle.reduce((acc: number, curr: any) => acc + curr.precio_solicitado, 0)
  }
  totalSolicitudAjuste() {
    ...
    this.cedulaDetalle.forEach(e => {
      ...
      Object.assign(e,{saldo_actual: e.saldo - e.ajuste})

    });
    this.totalCobro = this.cedulaDetalle.reduce((acc: number, curr: any) => acc + curr.ajuste, 0)
  ...
  armarcodigos() {
     ...
     this.documento.geografico = "0926";
      ...
      const documentoPrograma = this.documento.programa.descripcion || '';
...
/*
  ...
  updateIdp(){
    ...
    let nuevo = {
      ...
      partida: cedula.partida,
      denominacion: cedula.denominacion,
      disponible: cedula.disponible,
      valor_solicitado: 0
    }

    this.cedulaDetalle.push(nuevo);
    this.totalCobro = this.cedulaDetalle.reduce((acc: number, curr) => {return acc + parseFloat(curr.valor_solicitado)}, 0)
  ...
  removePresupuesto(index) {
    ...
    this.cedulaDetalle.splice(index, 1);
    this.totalCobro = this.cedulaDetalle.reduce((acc: number, curr) => {return acc + parseFloat(curr.valor_solicitado)}, 0)
  ...
  createRecDocumento() {
    ...
    if (this.totalCobro <= 0 || this.totalCobro == undefined) message += '* El Monto no puede ser menor o igual a 0.<br>';
    ...
    Swal.fire({
      icon: "warning",
      title: "¡Atención!",
      text:'¿Está a punto de generar un ICP de Nómina desea continuar?' ,
      showCloseButton: true,
        ...
        let datos = {
          ...
          /* Validamos si el periodo se encuentra aperturado */
            if (res["data"][0].estado !== 'C') {
              (this as any).mensajeSpinner = 'Generando ICP de Nómina...';
              this.lcargando.ctlSpinner(true);
              ...
              this.documento.cedulaDetalles = this.cedulaDetalle;
              //  this.cedulaDetalle.forEach(e => {
              //   if(e.valor_icp > 0){
              ...
              this.genIdpSvr.setGeneracionIdpNomina(data).subscribe(
                (res) => {
  ...
  modalCedulaPresupuestaria(){
    if(this.documento.periodo == undefined || this.documento.periodo ==''){
      this.toastr.info('Debe seleccionar un período')
    }else{
      const modal = this.modalService.open(ModalCedulaPresupuestariaComponent,{
        size:"xl",
        backdrop: "static",
        windowClass: "viewer-content-general",
      })
      modal.componentInstance.periodo = this.documento.periodo;
      modal.componentInstance.cedulaDetalle = this.cedulaDetalle;
    }
  }
  ...
  async ajustarIcpNomina(){
   ...
   // if (this.totalCobro > this.solicitud[0].valor) message += '* El monto no puede exceder al valor de la solicitud.<br>';
    ...
    this.documento.solicitud = this.solicitud;
    ...
    if (result.isConfirmed) {
      ...
      this.cierremesService.obtenerCierresPeriodoPorMes(datos).subscribe(async (res) => {
        ...
        if (res["data"][0].estado !=='C') {
          try {
            let response = await this.genIdpSvr.ajustarIcpNomina(this.documento.id_documento, this.documento);
            console.log(response)
