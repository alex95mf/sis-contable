...
export class FormCategoriasEspacioPublicitarioComponent implements OnInit {
  ...
  async validarCategoriasEspPublicitario() {
    if (this.esNuevaCatEspPub && this.permisos.guardar == "0") {
      this.srvTostar.warning("No tiene permisos para crear una Categoría de Espacios Publicitarios.", this.titulo);
    } else if (!this.esNuevaCatEspPub && this.permisos.editar == "0") {
        this.srvTostar.warning("No tiene permisos para editar una Categoría de Espacios Publicitarios.", this.titulo);
    } else {
      let resp = await this.validarDatos().then((respuesta) => {
          if (respuesta) {
            if (this.esNuevaCatEspPub) {
              ...
              let re: unknown = respuesta as string;
  ...
  validarDatos() {
    ...
    return new Promise((resolve, reject) => {
      if (
        ...
        message += 'Desea guardar una medida inicial menor que 0 ?'
        ...
        message += 'Desea guardar una medida final menor que 0 ?'
        ...
        message += 'Desea guardar un excedente inicial menor que 0 ?'
        ...
        message += 'Desea guardar un excedente final menor que 0 ?'
        ...
        message += 'Desea guardar un recargo que no está entre 0 y 1 ?'
        ...
        this.srvTostar.info("Debe ingresar una observación.");
        flag = true;
      }
      !flag ? resolve(message) : resolve(message);
    });
  }
  ...
  crearCategoriasEspPublicitario() {
    Swal.fire({
      icon: "warning",
      title: "¡Atención!",
      text: "¿Seguro que desea crear una nueva Categoría de Espacio Publicitario?",
      showCloseButton: true,
      showCancelButton: true,
      showConfirmButton: true,
      cancelButtonText: "Cancelar",
      ...
      if (result.isConfirmed) {
          this.texto_barra_carga = "Guardando Categoría de Espacios Publicitario..."
          this.barra_carga.ctlSpinner(true);
          ...
          this.srvCatEspPub.guardarCategoriasEspPublicitarios(datos).subscribe(
            (res) => {
              if (res["status"] == 1) {
                ...
                Swal.fire({
                  icon: "success",
                  title: "Categoría de Espacios Publicitarios Creada",
                  text: res['message'],
                  showCloseButton: true,
                  confirmButtonText: "Aceptar",
  ...
  actualizarCategoriasEspPublicitario() {
    Swal.fire({
      icon: "warning",
      title: "¡Atención!",
      text: "¿Seguro que desea editar esta Categoría de Espacios Publicitario?",
      showCloseButton: true,
      showCancelButton: true,
      showConfirmButton: true,
      cancelButtonText: "Cancelar",
          ...
          this.srvCatEspPub.actualizarCategoriasEspPublicitarios(datos, this.dato.id_categorias_esp).subscribe(
              (res) => {
                if (res["status"] == 1) {
                  ...
                  Swal.fire({
                    icon: "success",
                    title: "Categoría de Espacios Publicitario Actualizada",
                    text: res['message'],
                    showCloseButton: true,
                    confirmButtonText: "Aceptar",
                    confirmButtonColor: '#20A8D8'
