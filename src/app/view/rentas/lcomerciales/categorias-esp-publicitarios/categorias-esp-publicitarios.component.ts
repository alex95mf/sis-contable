...
export class CategoriasEspPublicitariosComponent implements OnInit {
  ...
  titulo = `Categorías Espacios Publicitarios`;
  botonera: any = [];
  datos_usuario: any;
  permisos: any;
  ...
  cargarCategoriasEspPublicitarios() {
    ...
    if (this.filtro .nombre !== undefined && !this.filtro .nombre.trim().length) {
...
//*
  ...
  cambiarMostrarInactivos(mostrarInactivo) {
    if (mostrarInactivo) {
      ...
      this.botonera[1].boton.texto = ' MOSTRAR INACTIVOS';
      this.filtro.estado = ['A'];
    } else {
      this.botonera[1].boton.icon = 'far fa-check-square';
      this.botonera[1].boton.texto = ' MOSTRAR ACTIVOS';
