...
export class GeneracionValorComponent implements OnInit {
  ...
  this.commonVarService.selectConceptoCustom.asObservable().subscribe(
    (res) => {
      ...
      this.conceptos.forEach(c => {
        if(c.codigo_detalle === "STAEP"){
          Object.assign(c, {  disabled:true })
        }else{
          Object.assign(c, { disabled:false })
        }
      })
      // this.calculateExoneraciones();
      this.calcSubtotal();
  ...
  getConceptos() {
    ...
    this.apiService.getConceptos().subscribe(
      res => {
        ...
        this.conceptosList = this.conceptosList.filter(c => c.codigo=="EP");
          ...
          let datos = {
            ...
            /* Validamos si el periodo se encuentra aperturado */
              if (res["data"][0].estado !== 'C') {
                  ...
                  //this.liquidacion.servtecadmin = this.conceptos.filter(e => e.codigo_detalle == 'STAEP')[0].total
  ...
  multiplicar(con_det) {
    const total = con_det.cantidad * con_det.valor;
    con_det.total = total;
    console.log(con_det);
    this.calcSubtotal();
  }
  ...
  expandConceptos() {
    ...
    modalInvoice.componentInstance.codigo = "EP";
    modalInvoice.componentInstance.conceptos = this.conceptos;
    modalInvoice.componentInstance.fTitle = "Conceptos por espectáculos públicos";
  ...
  expandListContribuyentes() {
    if (this.permissions.consultar == "0") {
      ...
      modalInvoice.componentInstance.permissions = this.permissions;
      modalInvoice.componentInstance.verifyRestore = this.verifyRestore;
      modalInvoice.componentInstance.validacion = 11;
    }
  }
