...
export class ModalExoneracionesComponent implements OnInit {
  ...
  cargarExoneraciones() {
    ...
    console.log(this.codigo_concepto)

    let data = {contribuyente: this.contribuyente.id, concepto: {codigo: this.codigo_concepto}, params: { filter: this.configuracion}}
  ...
  selectExoneracion(exon: any) {
    Object.assign(exon, {action: true})
    this.exoneracionesSelect = [{...exon}]
    this.closeModal(this.exoneracionesSelect);
  }

  closeModal(data?:any) {
    if(data){
      this.apiService.exoneracionesSelected$.emit(data);
    }
