...
@Component({
standalone: false,
  selector: 'app-anexos-list-enfermedad',
  templateUrl: './anexos-list-enfermedad.component.html',
...
export class AnexosListEnfermedadComponent implements OnInit {
    ...
    this.commonVarService.contribAnexoLoad.asObservable().subscribe(
      (res: any) => {
        ...
        let data = {
          ...
          component: myVarGlobals.fContribuyenteEnfermedad,
            ...
            this.apiService.getAnexos(data).subscribe(
              (res: any) => {
                ...
                if(res.data.length ==0){
                  this.commonVarService.disableCargarEnf.next({validacion: false, anexo: this.anexos,identificador:this.identificador})
                }else{
                  this.commonVarService.disableCargarEnf.next({validacion: true, anexo: this.anexos ,identificador:this.identificador})
                }
  ...
  deleteSinConfirmar(anexo){
    let data = {
      ...
      id_controlador: myVarGlobals.fContribuyenteEnfermedad,  // TODO: Actualizar cuando formulario ya tenga un ID
    ...
    this.apiService.deleteAnexo(data).subscribe(
      res => {
        ...
        this.commonVarService.disableCargarEnf.next({})
        // Swal.fire({
        //   title: this.fTitle,
