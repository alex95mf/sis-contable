...
@Component({
standalone: false,
  selector: 'app-anexos-list-tutor',
  templateUrl: './anexos-list-tutor.component.html',
  styleUrls: ['./anexos-list.component.scss']
})
export class AnexosListComponentTutor implements OnInit {
    ...
    this.commonVarService.contribAnexoLoad.asObservable().subscribe(
      (res: any) => {
        ...
        let data = {
          ...
          component: myVarGlobals.fContribuyenteTutor,
          identifier: res.id_cliente
        }

        
        if(res.condi == 'tutor' || res.condi === 'all'){
          console.log('Anexo tutor',res.condi)
          this.apiService.getAnexos(data).subscribe(
            (res: any) => {
              // console.log('Anexo tutor',res.data)
              ...
              //   this.commonVarService.disableCargarTutor.next({})
              // }
              console.log(res.data.length);
              if(res.data.length ==0){
                this.commonVarService.disableCargarTutor.next({validacion: false, anexo: this.anexos,identificador:this.identificador})
              }else{
                console.log('anexo tutor ')
                this.commonVarService.disableCargarTutor.next({validacion: true, anexo: this.anexos ,identificador:this.identificador})
              }
      ...
      res => {
        if (res.value) {
          let data = {
            ...
            id_controlador: myVarGlobals.fContribuyenteTutor,  // TODO: Actualizar cuando formulario ya tenga un ID
