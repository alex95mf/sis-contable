...
@Component({
standalone: false,
  selector: 'app-anexos-list-coo',
  templateUrl: './anexos-list-coop.component.html',
  styleUrls: ['./anexos-list.component.scss']
})
export class AnexosListComponentCoo implements OnInit {
    ...
    this.commonVarService.contribAnexoLoad.asObservable().subscribe(
      (res: any) => {
        ...
        let data = {
          ...
          component: myVarGlobals.fContribuyenteCoop,
        ...
        // console.log(data)
          ...
          if(res.condi == 'coop' || res.condi == 'all'){
            ...
            this.apiService.getAnexos(data).subscribe(
              (res: any) => {
                console.log('Anexo coop',res.data)
                ...
                if(res.data.length ==0){
                  this.commonVarService.disableCargarCoop.next({validacion: false, anexo: this.anexos,identificador:this.identificador})
                }else{
                  this.commonVarService.disableCargarCoop.next({validacion: true, anexo: this.anexos ,identificador:this.identificador})
                }
  ...
  deleteSinConfirmar(anexo){
    ...
    this.apiService.deleteAnexo(data).subscribe(
      res => {
        ...
        this.commonVarService.disableCargarCoop.next({})
        // Swal.fire({
        //   title: this.fTitle,
