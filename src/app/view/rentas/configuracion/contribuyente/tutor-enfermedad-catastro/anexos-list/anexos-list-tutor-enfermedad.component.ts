...
@Component({
standalone: false,
  selector: 'app-anexos-list-tutor-enfermedad',
  templateUrl: './anexos-list-tutor-enfermedad.component.html',
  styleUrls: ['./anexos-list.component.scss']
})
export class AnexosListTutorenfermedadComponent implements OnInit {
    ...
    this.commonVarService.contribAnexoLoad.asObservable().subscribe(
      (res: any) => {
        ...
        let data = {
          ...
          component: myVarGlobals.fContribuyenteTutorEnfermedad,
        ...
        if(res.condi == 'tutor' || res.condi === 'all'){
          ...
          this.apiService.getAnexos(data).subscribe(
            (res: any) => {
              ...
              //   this.commonVarService.disableCargarTutorEnf.next({})
              // }
              if(res.data.length ==0){
                this.commonVarService.disableCargarTutorEnf.next({validacion: false, anexo: this.anexos,identificador:this.identificador})
              }else{
                this.commonVarService.disableCargarTutorEnf.next({validacion: true, anexo: this.anexos ,identificador:this.identificador})
              }
