...
@Component({
standalone: false,
  selector: 'app-anexos-list-artesa',
  templateUrl: './anexos-list-artesa.component.html',
        ...
        if(res.condi == 'arte'||res.condi == 'all'){
          ...
          this.apiService.getAnexos(data).subscribe(
            (res: any) => {
              ...
              //   this.commonVarService.diableCargarArt.next({})
              // }
              if(res.data.length ==0){
                this.commonVarService.diableCargarArt.next({validacion: false, anexo: this.anexos,identificador:this.identificador})
              }else{
                this.commonVarService.diableCargarArt.next({validacion: true, anexo: this.anexos ,identificador:this.identificador})
              }
