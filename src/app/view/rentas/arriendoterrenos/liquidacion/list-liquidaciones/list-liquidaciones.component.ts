...
export class ListLiquidacionesComponent implements OnInit {
  ...
  constructor(
    ...
    private generacionSrv: ArriendoterrenoService,
  ) { }
  ...
  cargarLiquidaciones(flag: boolean = false){
    ...
    this.generacionSrv.getLiquidacionesAr(data).subscribe(
      (res) => {
  ...
  closeModal(data?:any) {
    if(data){
      this.commonVrs.selectListLiqArriendo.next(data);
    }
